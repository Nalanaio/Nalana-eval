...
class Judge:
    ...
    def judge(
        ...
        Empty-scene guard: when ``scene_snapshot.total_mesh_objects == 0`` we
        skip the call entirely. The screenshot in that case is the placeholder
        fallback PNG generated by ``make_fallback_png`` (black image with a
        text marker like "STATUS: NONE"). Multimodal judges have been observed
        to hallucinate full descriptions of the prompted concept ("a simple
        table with a flat top and four legs") when shown such blank images,
        producing inflated semantic / aesthetic scores that contaminate
        run-level metrics. See Task #21 / ADR-005 for the empirical evidence.

        ``scene_snapshot`` is optional for back-compat; when ``None``, the
        guard is bypassed (caller takes responsibility for not asking the
        judge to score garbage).
        """
        if case.judge_policy == JudgePolicy.SKIP:
            return None
        if scene_snapshot is not None and scene_snapshot.total_mesh_objects == 0:
            logger.info(
                "Judge skipped for %s: scene has 0 mesh objects (avoiding "
                "hallucination on placeholder PNG)", case.id,
            )
            return None
